...
void main()
{
  ...
  // for orto matrix, we don't need perspective division, you can remove it if you want; this is
  // general case;
  ...
  const bool outOfView =
    (shadowTexCoord.x < 0.0001f || shadowTexCoord.x > 0.9999f || shadowTexCoord.y < 0.0091f ||
     shadowTexCoord.y > 0.9999f);
  const float shadow =
    ((posLightSpaceNDC.z < textureLod(shadowMap, shadowTexCoord, 0).x + 0.001f) || outOfView)
    ? 1.0f
    : 0.0f;
