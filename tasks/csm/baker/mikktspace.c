...
static STSpace AvgTSpace(const STSpace* pTS0, const STSpace* pTS1)
{
  ...
  if (
    pTS0->fMagS == pTS1->fMagS && pTS0->fMagT == pTS1->fMagT && veq(pTS0->vOs, pTS1->vOs) &&
...
// degen triangles
static void DegenPrologue(
  STriInfo pTriInfos[], int piTriList_out[], const int iNrTrianglesIn, const int iTotTris);
...
tbool genTangSpace(const SMikkTSpaceContext* pContext, const float fAngularThreshold)
{
  ...
  if (
    pContext->m_pInterface->m_getNumFaces == NULL ||
  ...
  if (fSep >= fvMax[channel] || fSep <= fvMin[channel])
  {
    ...
    for (l = iL_in; l <= iR_in; l++)
    {
      ...
      while (l2 < l && bNotFound)
      {
        ...
        if (
          vP.x == vP2.x && vP.y == vP2.y && vP.z == vP2.z && vN.x == vN2.x && vN.y == vN2.y &&
          vN.z == vN2.z && vT.x == vT2.x && vT.y == vT2.y && vT.z == vT2.z)
          bNotFound = TFALSE;
        else
          ++l2;
      }
...
static void BuildNeighborsFast(
  STriInfo pTriInfos[], SEdge* pEdges, const int piTriListIn[], const int iNrTrianglesIn);
static void BuildNeighborsSlow(
  STriInfo pTriInfos[], const int piTriListIn[], const int iNrTrianglesIn);
...
{
  int f = 0, i = 0, t = 0;
  // pTriInfos[f].iFlag is cleared in GenerateInitialVerticesIndexList() which is called before this
  // function.
      ...
      if ((bIsDeg_a || bIsDeg_b) == TFALSE)
      {
        ...
        if (bOrientA != bOrientB)
        {
          ...
          if ((pTriInfos[t + 1].iFlag & GROUP_WITH_ANY) != 0)
            ...
            CalcTexArea(pContext, &piTriListIn[t * 3 + 0]) >=
...
static tbool AssignRecur(
  const int piTriListIn[], STriInfo psTriInfos[], const int iMyTriIndex, SGroup* pGroup);
  ...
  for (f = 0; f < iNrTrianglesIn; f++)
  {
    for (i = 0; i < 3; i++)
    {
      ...
      if ((pTriInfos[f].iFlag & GROUP_WITH_ANY) == 0 && pTriInfos[f].AssignedGroup[i] == NULL)
      {
        ...
        {
          const tbool bAnswer =
            AssignRecur(piTriListIn, pTriInfos, neigh_indexL, pTriInfos[f].AssignedGroup[i]);

          const tbool bOrPre2 =
            (pTriInfos[neigh_indexL].iFlag & ORIENT_PRESERVING) != 0 ? TTRUE : TFALSE;
        ...
        {
          const tbool bAnswer =
            AssignRecur(piTriListIn, pTriInfos, neigh_indexR, pTriInfos[f].AssignedGroup[i]);

          const tbool bOrPre2 =
            (pTriInfos[neigh_indexR].iFlag & ORIENT_PRESERVING) != 0 ? TTRUE : TFALSE;
          const tbool bDiff = bOrPre != bOrPre2 ? TTRUE : TFALSE;
          assert(bAnswer || bDiff);
          (void)bAnswer, (void)bDiff; /* quiet warnings in non debug mode */
        }
  ...
  const int piTriListIn[], STriInfo psTriInfos[], const int iMyTriIndex, SGroup* pGroup)
{
  ...
  if ((pMyTriInfo->iFlag & GROUP_WITH_ANY) != 0)
  {
    ...
    if (
      pMyTriInfo->AssignedGroup[0] == NULL && pMyTriInfo->AssignedGroup[1] == NULL &&
  ...
  for (g = 0; g < iNrActiveGroups; g++)
  {
    ...
    for (i = 0; i < pGroup->iNrFaces; i++) // triangles
    {
      ...
      for (j = 0; j < pGroup->iNrFaces; j++)
      {
        ...
        {
          const tbool bAny =
            ((pTriInfos[f].iFlag | pTriInfos[t].iFlag) & GROUP_WITH_ANY) != 0 ? TTRUE : TFALSE;
  ...
  STriInfo pTriInfos[], SEdge* pEdges, const int piTriListIn[], const int iNrTrianglesIn)
{
  ...
  for (i = 0; i < iEntries; i++)
  {
    ...
    GetEdge(
      &i0_A, &i1_A, &edgenum_A, &piTriListIn[f * 3], i0, i1); // resolve index ordering and edge_num
...
static void GetEdge(
  int* i0_out, int* i1_out, int* edgenum_out, const int indices[], const int i0_in, const int i1_in)
{
  *edgenum_out = -1;
  ...
  STriInfo pTriInfos[], int piTriList_out[], const int iNrTrianglesIn, const int iTotTris)
{
  ...
  while (t < iNrTrianglesIn && bStillFindingGoodOnes)
  {
    ...
    else
    {
      ...
      {
        const tbool bIsGood =
          (pTriInfos[iNextGoodTriangleSearchIndex].iFlag & MARK_DEGENERATE) == 0 ? TTRUE : TFALSE;
        if (bIsGood)
