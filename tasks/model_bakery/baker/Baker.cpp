...
Baker::Baker(std::filesystem::path path) : filepath(path) {}
...
std::optional<tinygltf::Model> Baker::loadFile() {
  ...
  success = loader.LoadASCIIFromFile(&model, &error, &warning,
  ...
  if (!model.extensions.empty() || !model.extensionsRequired.empty() ||
      !model.extensionsUsed.empty())
    ...
    for (const auto &prim : mesh.primitives) {
      ...
      std::array ptrs{
          reinterpret_cast<const std::byte *>(
              model.buffers[bufViews[0]->buffer].data.data()) +
              bufViews[0]->byteOffset + accessors[0]->byteOffset,
          reinterpret_cast<const std::byte *>(
              model.buffers[bufViews[1]->buffer].data.data()) +
              bufViews[1]->byteOffset + accessors[1]->byteOffset,
          hasNormals ? reinterpret_cast<const std::byte *>(
                           model.buffers[bufViews[2]->buffer].data.data()) +
                           bufViews[2]->byteOffset + accessors[2]->byteOffset
                     : nullptr,
          hasTangents ? reinterpret_cast<const std::byte *>(
                            model.buffers[bufViews[3]->buffer].data.data()) +
                            bufViews[3]->byteOffset + accessors[3]->byteOffset
                      : nullptr,
          hasTexcoord ? reinterpret_cast<const std::byte *>(
                            model.buffers[bufViews[4]->buffer].data.data()) +
      ...
      std::array strides{
          ...
          hasTangents
              ...
              : 0,
          hasTexcoord
              ? (bufViews[4]->byteStride != 0
                     ? bufViews[4]->byteStride
                     : tinygltf::GetComponentSizeInBytes(
                           accessors[4]->componentType) *
      ...
      for (std::size_t i = 0; i < vertexCount; ++i) {
        ...
        // NOTE: it's faster to do a template here with specializations for all
        ...
        vtx.positionAndNormal = glm::vec4(
            pos, std::bit_cast<float>(encodeNormalized(glm::vec4(normal, 0))));
        vtx.texCoordAndTangentAndPadding = glm::vec4(
            texcoord, std::bit_cast<float>(encodeNormalized(tangent)), 0);
...
void Baker::changeBuffer(tinygltf::Model &model, BakedMeshes &baked_meshes) {
  ...
  std::memcpy(bakedBuffer.data.data(), baked_meshes.indices.data(),
              baked_meshes.indices.size() * sizeof(uint32_t));
  std::memcpy(bakedBuffer.data.data() + indicesBufferSize,
              baked_meshes.vertices.data(),
              baked_meshes.vertices.size() * sizeof(Vertex));
  model.buffers.push_back(bakedBuffer);
}

// void Baker::changeBufferViews(tinygltf::Model& model, BakedMeshes&
// baked_meshes) {
...
//   indicesBufferView.byteLength = baked_meshes.indices.size() *
//   sizeof(uint32_t); indicesBufferView.byteStride = 0;
...
//   vertexBufferView.byteOffset = baked_meshes.indices.size() *
//   sizeof(uint32_t); vertexBufferView.byteLength =
//   baked_meshes.vertices.size() * sizeof(Vertex); vertexBufferView.byteStride
//   = sizeof(Vertex); vertexBufferView.target = TINYGLTF_TARGET_ARRAY_BUFFER;

//   model.bufferViews.push_back(indicesBufferView);
//   model.bufferViews.push_back(vertexBufferView);
// }

void Baker::modifyBufferViews(tinygltf::Model &model,
                              BakedMeshes &baked_meshes) {
  std::size_t bufferViewOffset = 0;
  // tinygltf::BufferView previousBufferView =
  //     {};
  for (auto &bufferView : model.bufferViews) {
    if (bufferView.target == TINYGLTF_TARGET_ELEMENT_ARRAY_BUFFER) {
      bufferView.name = "indices_baked";
      bufferView.buffer = 0;
      bufferView.byteOffset = 0; // assumption - indices buffer view always comes first
      bufferView.byteLength = baked_meshes.indices.size() * sizeof(uint32_t);
      bufferView.byteStride = 0;

      bufferViewOffset = bufferView.byteLength;
    }
    if (bufferView.target == TINYGLTF_TARGET_ARRAY_BUFFER) {
      bufferView.buffer = 0;
      bufferView.byteOffset = bufferViewOffset;
      bufferView.byteLength = (bufferView.byteLength / bufferView.byteStride) * sizeof(Vertex);
      bufferView.byteStride = sizeof(Vertex);

      bufferViewOffset = bufferView.byteOffset + bufferView.byteLength;
    }
  }
}

// void Baker::modifyAccessors(tinygltf::Model &model, BakedMeshes &baked_meshes) {
...
void Baker::saveFormatted(tinygltf::Model &model) {
  ...
  if (!loader.WriteGltfSceneToFile(&model, newFile, false, false, true,
                                   false)) {
    spdlog::error("Error occured when saving formatted file! Location - {}",
                  newFile);
  }
}
...
uint32_t Baker::encodeNormalized(glm::vec4 normal) const {
  ...
  uint32_t newX = (std::lround(normal.x * scale) + 128);
  uint32_t newY = ((std::lround(normal.y * scale) + 128) << 8);
  uint32_t newZ = ((std::lround(normal.z * scale) + 128) << 16);
  uint32_t newW = ((std::lround(normal.w * scale) + 128) << 24);
