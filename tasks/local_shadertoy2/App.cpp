...
void App::loadTextures() {
  ...
  unsigned char *swordTextureData =
      stbi_load(LOCAL_SHADERTOY2_TEXTURES "test_tex_1.png", &width, &height,
  ...
  etna::Buffer swordTextureBuffer =
  ...
  transferHelper->uploadBuffer(*oneShotCommands, swordTextureBuffer, 0,
                               std::as_bytes(source));
  ...
  {
    etna::set_state(commandBuffer, swordTexture.get(),
    ...
    vk::BufferImageCopy copyRegion = {
        ...
        .imageSubresource = {.aspectMask = vk::ImageAspectFlagBits::eColor,
                             ...
                             .baseArrayLayer = 0,
                             .layerCount = 1},
        .imageExtent = vk::Extent3D{static_cast<uint32_t>(width),
                                    static_cast<uint32_t>(height), 1}};

    commandBuffer.copyBufferToImage(
        swordTextureBuffer.get(), swordTexture.get(),
...
//       path + "moonlight_back.bmp",  path + "moonlight_down.bmp",
//       path + "moonlight_front.bmp", path + "moonlight_left.bmp",
//       path + "moonlight_right.bmp", path + "moonlight_up.bmp",
//   };
...
void App::drawFrame() {
  ...
  if (nextSwapchainImage) {
    ...
    {
      // First of all, we need to "initialize" th "backbuffer", aka the current
      // swapchain image, into a state that is appropriate for us working with
      // it. The initial state is considered to be "undefined" (aka "I contain
      // trash memory"), by the way. "Transfer" in vulkanese means "copy or
      ...
      // "action", i.e. compute dispatches and blit/copy operations.
      etna::flush_barriers(currentCmdBuf);

      // TODO: Record your commands here!

      etna::set_state(currentCmdBuf, generatedTexture.get(),
      ...
      // Begin generated texture render pass
      {
        ...
        currentCmdBuf.pushConstants<PushConstants>(
            ...
            vk::ShaderStageFlagBits::eFragment, 0, {pushConst});
      ...
      // Begin final image render pass
      {
        ...
        auto set = etna::create_descriptor_set(
            graphicShaderInfo.getDescriptorLayoutId(0), currentCmdBuf,
            {etna::Binding{0, generatedTexture.genBinding(
                                  sampler.get(),
                                  vk::ImageLayout::eShaderReadOnlyOptimal)},
             etna::Binding{1, swordTexture.genBinding(
                                  sampler.get(),
                                  vk::ImageLayout::eShaderReadOnlyOptimal)}});
      ...
      // At the end of "rendering", we are required to change how the pixels of
